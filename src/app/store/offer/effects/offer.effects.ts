...
export class OfferEffects {
  ...
  loadOffers$ = createEffect(() =>
    this.actions$.pipe(ofType(OfferActions.loadOffers), switchMap(() => this.offerApiService.getOffers()
      .pipe(map(offers => OfferActions.loadOffersSuccess({offers})),
        catchError((error: HttpErrorResponse) => of(OfferActions.loadOffersFailure({error: error.message})))))))
}
